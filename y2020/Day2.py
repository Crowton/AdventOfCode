...
for _ in range(1000):
    ...
    numA, numB = map(int, count.split("-"))
    data.append((numA, numB, letter, password))


# Part 1
validCount = 0
for letterMin, letterMax, letter, password in data:
    if letterMin <= password.count(letter) <= letterMax:
        validCount += 1
print(validCount)
...
validCount = 0
validCount2 = 0
for posA, posB, letter, password in data:
    if [password[posA - 1], password[posB - 1]].count(letter) == 1:
        validCount += 1
    if (password[posA - 1] == letter) ^ (password[posB - 1] == letter):
        validCount2 += 1
print(validCount, validCount2)

# Part 2 comprehension
# print(sum((password[posA - 1] == letter) ^ (password[posB - 1] == letter) for posA, posB, letter, password in data))


# Single comprehension solving both tasks
# print([sum(l) for l in zip(*[
#         (numA <= password.count(letter) <= numB, (password[numA - 1] == letter) ^ (password[numB - 1] == letter))
#         for _ in range(1000)
#         for policy, password in [input().split(": ")]
#         for count, letter in [policy.split()]
#         for numA, numB in [map(int, count.split("-"))]
#     ])
# ])

# Collapsed comprehension
# print([sum(l) for l in zip(*[(na <= psw.count(let) <= nb, (psw[na - 1] == let) ^ (psw[nb - 1] == let)) for _ in range(1000) for pol, psw in [input().split(": ")] for c, let in [pol.split()] for na, nb in [map(int, c.split("-"))]])])
