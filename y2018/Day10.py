...
for i in range(391):
    points.append(list(map(int, input().replace("position=<", "").replace("> velocity=<", ",").replace(">", "").split(","))))

for i in range(20000):
    tempP = set()
    for j in range(len(points)):
        p = points[j]
        points[j] = [p[0]+p[2], p[1]+p[3], p[2], p[3]]
        tempP.add((p[0]+p[2], p[1]+p[3]))

    found = 0
    for p in tempP:
        if (p[0], p[1]+1) in tempP or (p[0], p[1]-1) in tempP or (p[0]+1, p[1]) in tempP or (p[0]-1, p[1]) in tempP:
            found += 1

    if found > 50:
        minX = min([p[1] for p in tempP])
        maxX = max([p[1] for p in tempP])
        minY = min([p[0] for p in tempP])
        maxY = max([p[0] for p in tempP])
